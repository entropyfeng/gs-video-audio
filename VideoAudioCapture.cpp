...
gboolean gst_message_print(GstBus* bus, GstMessage* message, gpointer user_data)
{
    switch (GST_MESSAGE_TYPE (message))
    {
        ...
        default:
        {
            spdlog::error("gstreamer {} message {} ==> {}", GST_OBJECT_NAME(message->src), gst_message_type_get_name(GST_MESSAGE_TYPE(message)), GST_OBJECT_NAME(message->src));
            break;
        }
    }
...
/*    // enqueue the buffer for color conversion
    ...
    {
        spdlog::warn("gstDecoder -- resolution changing from ({0}x{1}) to ({2}x{3})", width, height, mOptions.width, mOptions.height);
